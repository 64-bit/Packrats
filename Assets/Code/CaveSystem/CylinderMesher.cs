...
namespace Packrats
{
    public static class CylinderMesher
    {
        ...
        public static void AppendWalls(ref NativeMeshData meshData, int radialSegments,
            float startRadial, float radialLength, float radius, float yPosition, float height)
        {
            int indexOffset = meshData.Positions.Length;

            //In increasing 'radians' position, sweep around creating collumns, with the top vertex being first.
            int columns = radialSegments + 1;
            float radianStepSize = radialLength / radialSegments;

            float initialAngle = startRadial;
            ...
            for (int i = 0; i < columns; i++)
            {
                ...
                //Write out UV
                float uvX = ((float)i / (float)radialSegments);
                meshData.UVs.Add(new float2(uvX, 0.0f));
                meshData.UVs.Add(new float2(uvX, 1.0f));
            }
